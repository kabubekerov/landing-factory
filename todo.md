...
- [ ] Make `Стратегическая встреча по переупаковке` black.
- [ ] Remove the text `Оставьте ссылку на проект или сайт застройщика — и мы покажем, можно ли собрать для него более точную сегментную версию.`
- [ ] Remove the text `Проведём встречу и покажем, как можно переупаковать ваш ЖК через сегмент, JTBD и структуру страницы, которая помогает продавать не общий рынок, а точную зрелую семейную задачу.`
- [ ] Change the form button text to `запросить консультацию`.
- [ ] Remove the text `Если текущее позиционирование не раскрывает силу проекта, оставьте контакты — покажем вариант переупаковки.`
- [ ] Change `Посмотреть, как можно переупаковать этот ЖК под зрелую семью` to `запросить консультацию`.
